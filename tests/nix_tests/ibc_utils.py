...
def get_evmos_generator(
    ...
    if is_rocksdb:
        ...
        if custom_scenario:
            ...
            modified_bin = build_patched_osd(custom_scenario)
            post_init_func = update_evmos_bin(modified_bin)
            if stride_included:
                post_init_func = update_osd_and_setup_stride(modified_bin)
            gen = setup_custom_evmos(
                tmp_path,
                26700,
                Path(__file__).parent / file,
